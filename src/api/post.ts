...
export async function createPostWithImage({ message, mediaType, postType, mentions, media }: { message: string; mediaType: string; postType: string; mentions: string[]; media: File[] }) {
  ...
  media.forEach((file) => formData.append('media', file)); // Append each file as a separate 'media' field
...
export async function editPost(postId: string, { message, postType }: { message: string; postType: string }) {
  const token = localStorage.getItem('token');
  return axios.put(`${import.meta.env.VITE_API_BASE_URL}/posts/${postId}`, { message, postType }, {
    headers: {
...
export async function addComment(postId: string, { text, mentions }: { text: string; mentions: string[] }) {
  const token = localStorage.getItem('token');
  return axios.post(`${import.meta.env.VITE_API_BASE_URL}/posts/${postId}/comment`, { text, mentions }, {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
    },
...
export async function deleteComment(postId: string, commentId: string) {
  const token = localStorage.getItem('token');
  return axios.delete(`${import.meta.env.VITE_API_BASE_URL}/posts/${postId}/comment/${commentId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
